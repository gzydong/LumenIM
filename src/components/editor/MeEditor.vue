<template>
  <div>
    ...
    <!-- 图片查看器 -->
    <me-editor-image-view v-model="imageViewer.isShow" :file="imageViewer.file" @confirm="confirmUploadImage"
      ref="imageViewer" />

    <me-editor-recorder v-if="recorder" @close="recorder = false" />

    <!-- 代码块编辑器 -->
    <talk-code-block v-if="codeBlock.isShow" :edit-mode="codeBlock.editMode" @close="codeBlock.isShow = false"
  ...
  export default {
    ...
    methods: {
      ...
      confirmUploadImage() {
        ...
        ServeSendTalkImage(fileData).then(res => {
          ref.loading = false;
          if (res.code == 200) {
            ref.closeBox();
          }
